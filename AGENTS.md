...
## Package manager
- Use **pnpm** only (`pnpm install`, `pnpm run <script>`). Do not add `package-lock.json` or use npm/yarn for this repo.
- Lockfile: `pnpm-lock.yaml` (commit when dependencies change).

## Verify before committing
- Typecheck + lint + tests pass (`pnpm run build`, `pnpm test`, `pnpm run lint`).
